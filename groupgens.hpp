...
// symmetric group is like {{(1 2)}, {(1 2 3 4 5)}}
...
// alternating group is like {{(1 2 3)}, {(1 2 3 4 5)}}
template <typename Domain> auto alternating_gens() {
  assert(Domain::fin - Domain::start > 2);
  gens_t<Domain> retval = cyclic_gens<Domain>();
  PermLoop<Domain> triple = {Domain::start, Domain::start + 1,
                             Domain::start + 2};
  retval.push_back({triple});
  return retval;
}
